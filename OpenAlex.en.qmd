...
execute:
  ...
  error: false
webr:
  packages: ['dplyr','tidyr','stringr','ggplot2']
lightbox: true
---
...
The [OpenAlex REST API](https://developers.openalex.org/api-reference/introduction) allows programmatic access to and retrieval of OpenAlex data[^1]. To interact with the API you are required to register an [API key](https://openalex.org/login?redirect=/settings/api-key) as of February 2026 [@Priem2026]. The API is designed as a freemium service; more detailed information on the pricing model is provided in the [OpenAlex technical documentation](https://developers.openalex.org/api-reference/authentication). However, the free daily quota is sufficient for our use case in this notebook.
...
OpenAlex additionally offers a [data snapshot](https://developers.openalex.org/download/overview#openalex-snapshot) with a copy of the complete database for download, which a user can then load into their own data warehouse or relational database. The snapshot gets updated monthly. 
...
OpenAlex offers a lot more information than what we are exploring in this notebook. For a full documentation of all available entities and fields, please visit the [OpenAlex technical documentation](https://developers.openalex.org/api-reference/introduction).

## Loading packages

We will load the *openalexR* package [@openalexR] that allows us to query the OpenAlex API from within our notebook. We will also load the packages *dplyr* [@dplyr], *tidyr* [@tidyr], *stringr* [@stringr] and *ggplot2* [@ggplot2] that provide a lot of additional functionalities for data wrangling and visualization and are part of the *tidyverse* package [@tidyverse].
...
::: {.callout-note}
We won't query the API in this notebook directly. We ran the API query once and stored the results in a separate file we will load into the notebook in a following code chunk.
:::
...
We have saved the query result as an rds-file, which we will now load. You can skip this step if you have adapted the notebook to investigate the publication output of your institution.
...
```{r}
df |>
  dplyr::group_by(host_organization, host_organization_name) |>
  dplyr::summarise(n = n()) |>
...
df |>
  ...
  dplyr::summarise(n = n()) |>
  dplyr::arrange(desc(n)) |>
  dplyr::filter(grepl("^Springer|Nature", host_organization_name))
```
...
```{r}
df |>
  dplyr::mutate(host_organization_name = stringr::str_replace_all(host_organization_name, "^Springer.*$|Nature.*$", "Springer Nature")) |>
...
```{r}
df |>
  tidyr::unnest_wider(apc, names_sep = ".") |>
...
df |>
  tidyr::unnest_wider(apc, names_sep = ".") |>
  tidyr::unnest_longer(c(apc.type, apc.value, apc.currency, apc.value_usd, apc.provenance)) |>
  dplyr::select(id, starts_with("apc"))
```
...
df |>
  tidyr::unnest_wider(apc, names_sep = ".") |>
  tidyr::unnest_longer(c(apc.type, apc.value, apc.currency, apc.value_usd, apc.provenance)) |>
  tidyr::pivot_wider(id_cols = id, names_from = apc.type, values_from = c(apc.value, apc.currency, apc.value_usd, apc.provenance))
```
...
```{r}
df |>
  dplyr::filter(!is.na(is_oa)) |>
  dplyr::group_by(source_display_name) |>
  dplyr::summarise(
    n_articles = n(),
    n_oa_articles = sum(is_oa),
    n_closed_articles = n_articles - n_oa_articles
  ) |>
  dplyr::arrange(desc(n_oa_articles))
```
...
```{r}
df |>
  dplyr::filter(source_display_name %in% c("Scientific Reports", "Astronomy and Astrophysics", "Journal of High Energy Physics")) |>
  dplyr::group_by(source_display_name, is_oa, oa_status) |>
  dplyr::summarise(n = n())
```
...
```{r}
df |>
  dplyr::group_by(publication_year, oa_status) |>
...
```{webr-r}
df |>
  dplyr::filter(host_organization_name %in% c("Wiley", "Elsevier BV", "Springer Science+Business Media")) |>
  dplyr::group_by(host_organization_name, oa_status) |>
  dplyr::summarise(count = n()) |>
  dplyr::mutate(perc = count / sum(count) * 100) |>
