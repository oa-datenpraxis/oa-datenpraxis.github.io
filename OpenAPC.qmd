...
We will load the *dplyr* [@dplyr] and *tidyr* [@tidyr] packages that provide a lot of additional functionalities for data wrangling, and *ggplot2* [@ggplot2] for data visualization. All three packages are part of the *tidyverse* package [@tidyverse].
...
```{r}
df |>
  dplyr::group_by(journal_full_title) |>
  dplyr::summarise(
    n_articles = n(),
    sum_apc = sum(euro),
...
```{r}
df |>
  dplyr::filter(journal_full_title %in% c("Scientific Reports", "PLOS ONE", "Frontiers in Immunology")) |>
...
```{r}
df |>
  dplyr::group_by(period, is_hybrid) |>
  dplyr::summarise(n_articles = n()) |>
...
## 2. Visuzlization of APC distribution by publisher
...
```{webr-r}
df |>
  dplyr::filter(publisher %in% c("Springer Nature", "Wiley-Blackwell", "Frontiers Media SA")) |>
